...
REGION_PATTERNS={'서울':re.compile(r'(?<![가-힣])서울(?:특별시)?\s'),'경기':re.compile(r'(?<![가-힣])경기(?:도)?\s')}
...
async def enable_current_only(p):
    ...
    try:
        async with p.expect_navigation(wait_until='domcontentloaded',timeout=15000): await cur.click(force=True)
    except Exception: pass
    ...
    if not await p.evaluate("()=>typeof window.jQuery==='function'"): raise RuntimeError('jQuery unavailable for current-only fallback')
    await p.evaluate("""()=>{const $=window.jQuery,$c=$('#jobs_ving_chk');if(!$c.length)throw new Error('jobs_ving_chk missing');$c.prop('checked',false);$c.trigger('click');}""")
...
async def run(b,region,code):
    ...
    try:
        ...
        await p.wait_for_timeout(300);radio=p.locator(f'#area_level_{code}')
        if not await radio.count():raise RuntimeError('region radio missing')
        await radio.evaluate('e=>{e.click();e.dispatchEvent(new Event("change",{bubbles:true}))}');await p.wait_for_timeout(600)
        ...
        await all_radio.evaluate('e=>{e.click();e.dispatchEvent(new Event("change",{bubbles:true}))}');await p.wait_for_timeout(250)
        selector_vals=await p.locator('input[name="area_selector_val"]').evaluate_all('els=>els.map(e=>e.value)');expected=f'2000-{code}'
        if expected not in selector_vals:raise RuntimeError(f'expected area_selector_val {expected} not created: {selector_vals}')
        if not await visible_click(p.locator('#btn_area_ok')):raise RuntimeError('region selection confirm button missing')
        await p.wait_for_timeout(400);array_vals=await p.locator('input[name="array_area_type"]').evaluate_all('els=>els.map(e=>e.value)')
        if expected not in array_vals:raise RuntimeError(f'expected array_area_type {expected} not committed: {array_vals}')
        current_path=await enable_current_only(p);array_vals=await p.locator('input[name="array_area_type"]').evaluate_all('els=>els.map(e=>e.value)')
        ...
        other='경기' if region=='서울' else '서울';hits=sum(bool(REGION_PATTERNS[region].search(x)) for x in rows);contam=sum(bool(REGION_PATTERNS[other].search(x)) for x in rows);ended=sum(('마감' in x or '종료' in x) and '진행중' not in x for x in rows)
