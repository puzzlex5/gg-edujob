...
def parse_rows(html,surface):
 ...
 for tr in soup.find_all('tr'):
  ...
  location=next((c for c in cells if token in c),token)
  deadline=next((c for c in cells if re.search(r'(?:D-\d+|상시채용|채용시까지|~\s*\d{1,2}\.\d{1,2})',c)), '')
  out.append({'sourceIdentity':f'jobteacher:{m.group(1)}','sourceId':m.group(1),'source':'잡티처','sourceType':'민간 구인','sourceSurface':'academy-recruitment','sourceSurfaceLabel':'잡티처','province':token,'location':location,'title':title,'applyEndText':deadline,'registered':TODAY.isoformat(),'url':href,'rawRowText':text[:900]})
 return out
...
def main():
  ...
  for surface,start in SURFACES.items():
   try:
    ...
    for n in range(1,last+1):
     ...
     for j in new: seen.add(j['sourceIdentity']); rows.append(j); discovered[j['sourceIdentity']]=j
 ...
 for sid,j in discovered.items(): entries[sid]={'sourceId':j['sourceId'],'url':j['url'],'lastSeenAt':now,'presentInLatestScan':True,'province':j['province']}
 ...
 report={'generatedAt':now,'source':'잡티처','policy':'jobteacher-active-browser-v1','healthy':healthy,'traversalComplete':traversal_complete,'sourceReports':reports,'candidateIdCount':len(discovered_ids),'publishedIdCount':len(published_ids),'missingAfterCount':len(missing),'missingAfter':missing[:100],'errors':errors,'previousPublishedCount':prev_count,'severeDrop':severe_drop,'preservedPreviousDataset':not healthy and bool(prev_jobs),'publicationEnabled':False,'nextGate':'detail-link validation + generic unified-search adapter'}
