...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        def on_response(resp):
            try:
                ...
                for m in API_RE.finditer(text):
                    ...
                    contexts.append({"apiCandidate": candidate, "context": compact_context(text, m.start(), m.end())})
                if matches:
                    script_findings.append({"scriptUrl": resp.url, "status": resp.status, "apiCandidates": sorted(matches)[:200], "apiContexts": contexts[:80]})
        ...
        try:
            ...
            if not blocked:
                for selector in ("a:has-text('채용정보')", "button:has-text('채용정보')", "[role='button']:has-text('채용정보')", "nav a:has-text('채용')", "header a:has-text('채용')"):
    ...
    report = {
        "generatedAt": generated_at, "source": "훈장마을", "publiclyReachable": bool(script_urls) and not blocked,
        "humanVerificationDetected": blocked, "scriptCount": len(set(script_urls)), "scriptsWithRecruitApiHints": len(script_findings),
        "observedRecruitApiStrings": observed, "generalListApiStringCandidates": general_candidates, "findings": script_findings[:100], "errors": errors[:100],
        "safety": {"tlsVerificationDisabled": False, "authenticationBypassed": False, "captchaBypassed": False, "publicationAttempted": False},
