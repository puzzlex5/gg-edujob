#!/usr/bin/env python3
"""Deep-discover recruitment details on selected Seoul support-office CMS boards.

Some Seoul support offices expose recruitment posts on CMS boards in addition to the
standard FUS/JOL11 surface. This crawler traverses independently verified CMS list boards
until the 90-day lookback is crossed (or a proven natural end is reached), records exact
same-host detail URLs, and fails closed on access/pagination ambiguity.

It never publishes jobs itself. The existing conservative CMS recovery step re-fetches and
validates each discovered detail page before any row can enter jobs.json.
"""
...
# These are alternative recruitment surfaces already observed on official support-office hosts.
# Keep them explicit: guessing CMS menu paths is less safe than failing closed when a verified
# board moves or changes structure.
BOARDS = [
    {
        "office": "동부교육지원청",
...
def crawl_board(spec: dict):
    ...
    for page_no in range(1, MAX_PAGES + 1):
        ...
        if not nxt:
            # A CMS board with no advertised total and no next-page link can be a genuine
            # one-page board. If a total is advertised, however, a missing next link is a hard
            # pagination failure and must never be treated as complete.
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "boards": [
            {k: row[k] for k in (
                "office", "pagesScanned", "rowsSeen", "acceptedRecentRecruitment",
                "stopReason", "coverageComplete", "error"
            )}
            for row in board_reports
        ],
    }, ensure_ascii=False))
