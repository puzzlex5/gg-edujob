...
# Evidence audit: compare the historical 7,424 support records with the current 7,353 set.
...
def classify(old: dict, current: list[dict], current_ids: set[str]) -> dict:
    ...
    old_title = title(old); old_url = url(old)
    ...
    if r is not None:
        final_url = r.url; body = re.sub(r"\s+", " ", r.text)
    ...
    apply_end = str(old.get("applyEnd") or old.get("deadline") or "")
    return {"oldId": oid, "classification": "parser/기간 기준 변경" if apply_end and apply_end < datetime.now(timezone.utc).date().isoformat() else "정상 종료/삭제", "title": old_title, "url": old_url, "probe": status}


def main():
    old_all = git_json(HIST, "jobs.json")
    cur_all = json.loads(Path("jobs.json").read_text(encoding="utf-8"))
    old = [j for j in old_all if is_support(j)]; cur = [j for j in cur_all if is_support(j)]
    current_ids = {identity(j) for j in cur}; missing = [j for j in old if identity(j) not in current_ids]
