#!/usr/bin/env python3
"""Audit the historical 7424 -> current 7353 support population using production contracts."""
...
from update_collector_status import stable_source_id
from support_population_contract import is_support_population_job, contract_metadata
...
def is_support(job,as_of): return is_support_population_job(job,as_of=as_of)
def identity(job): return stable_source_id(job)
...
def classify(old,current,current_ids):
    ...
    if len(same)==1:
        j=same[0]; return {"oldId":oid,"classification":"ID/URL 이동","title":old_title,"url":old_url,"newId":identity(j),"newUrl":url(j),"newTitle":title(j)}
    ...
    if r is not None:
        ...
        if end and end < datetime.now(timezone.utc).date().isoformat():return {"oldId":oid,"classification":"parser/기준 변경","title":old_title,"url":old_url,"http":r.status_code,"finalUrl":final}
...
def main():
    # One execution timestamp is shared by both snapshots, matching the production 90-day contract.
    as_of=datetime.now(timezone.utc).astimezone(__import__('datetime').timezone(__import__('datetime').timedelta(hours=9)))
    old_raw=[j for j in as_jobs(git_json(HIST,"jobs.json")) if is_support(j,as_of)]
    cur_raw=[j for j in as_jobs(json.loads(Path("jobs.json").read_text(encoding="utf-8"))) if is_support(j,as_of)]
    old,old_collisions,old_failures,old_failure_examples=build(old_raw); cur,cur_collisions,cur_failures,cur_failure_examples=build(cur_raw)
    ...
    report={"generatedAt":as_of.isoformat(),"historicalCommit":HIST,"populationContract":contract_metadata(as_of=as_of),"rawHistoricalSupportCount":len(old_raw),"rawCurrentSupportCount":len(cur_raw),"historicalSupportCount":len(old),"currentSupportCount":len(cur),"intersectionCount":len(intersection),"oldOnlyCount":len(old_only),"currentOnlyCount":len(current_only),"normalizedCollisionCount":{"historical":old_collisions,"current":cur_collisions},"identityParseFailureCount":{"historical":old_failures,"current":cur_failures},"identityParseFailureExamples":{"historical":old_failure_examples,"current":cur_failure_examples},"historicalMinusCurrent":len(missing),"counts":counts,"actualMissing":[],"rows":rows,"targetPopulation":{"historical":7424,"current":7353,"difference":71},"targetPopulationMatches":len(old)==7424 and len(cur)==7353 and len(old_only)==71,"healthy":len(old)==7424 and len(cur)==7353 and len(old_only)==71 and counts.get("실제 누락 확정",0)==0}
